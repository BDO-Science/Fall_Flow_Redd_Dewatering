...
kwk_webpage <- read_html('https://www.cbr.washington.edu/sacramento/tmp/riverdaily_1693937007_339.html')
...
for(a in 1:(length(kesFlow)-1)){
  for(r in 1: nrow(kesFlowReal)){
    if(is.na(kesFlowReal$KES[r])){
      ...
      kesFlowReal[r, paste0("A", substr(names(kesFlow)[a + 1], 4,6), "cfs")] <- kesFlowReal$KES[r]
...
flows <- kesFlowReal %>% rename('Alt-1b' = 'A.1bcfs', 'Alt-3c' = 'A.3ccfs', 
                                'Alt-3d' = 'A.3dcfs', 'Alt-3f' = 'A.3fcfs', 'Alt-3g' = 'A.3gcfs') %>%
  gather(key = 'Alternates', value = 'Flow', 'Alt-1b', 'Alt-3c', 'Alt-3d', 'Alt-3f', 'Alt-3g') %>% 
