...
reddsTable <- reddsTable %>% rename('Date' = 'Estimated.Date.of.Emergence', 'Flow' = 'ACTUAL.or.ESTIMATED..DEWATER.FLOW') %>% mutate(cols = factor(cols, levels = c('Emerged', 'Remaining', 'Dewatered')))
...
ggplot() + geom_line(flows, mapping = aes(x = Date, y = Flow, color = Alts), size = 0.75) +
  ...
  scale_fill_manual(labels = c('Emerged', 'Remaining', 'Dewatered'), values = c('#33CCFF', 'darkgrey', '#CC6600')) +
