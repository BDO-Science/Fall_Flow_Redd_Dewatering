...
There is no real time data on fall-run redd counts. Estimates are predicted based estimated dewatering percentages from USFWS (2006) and spring-run and fall-run spawn timing based on fresh female carcasses encountered by week, 2003 through 2022. Estimated emergence dates based on river water temperatures during fall-run period at CCR gauge in 2011 that closely mimics 2023 summer water temperatures. Fall-run dewatered redd estimates range from **`r min(summary[2]) `** to **`r max(summary[2])`%**.
...
for(a in 1:(length(kesFlow)-1)){
 ...
 names(dflist) = dfnames # naming the list 
  ...
  total_volume_sf <- round((sum(as.numeric(kesFlow[kesFlow$Date >= "2023-09-01" & kesFlow$Date < "2024-02-01" , names(kesFlow)[a + 1]]))*1.983)/1000, 0)
 ...
 dflist[[a]] <- as.character(c(avg_sept_flow, avg_oct_flow, total_volume_ao, total_volume_sf, 
...
flows <- kesFlowReal %>% rename('1b' = 'A.1bcfs', '3c' = 'A.3ccfs', 
                                '3d' = 'A.3dcfs', '3f' = 'A.3fcfs', '3g' = 'A.3gcfs') %>%
  gather(key = 'Alts', value = 'Flow', '1b', '3c', '3d', '3f', '3g') %>% 
  mutate(Flow = as.numeric(Flow)) %>%
  filter(Date > "2023-08-31")
todays_date <- as.Date(format(Sys.Date(), '%Y-%m-%d'))

ggplot() + geom_line(flows, mapping = aes(x = Date, y = Flow, color = Alts), size = 0.75) +
  geom_line(kwk_flows, mapping = aes(x = Date, y = as.numeric(KWK)), size = 0.75) +
  geom_line(kes_flows, mapping = aes(x = Date, y = as.numeric(KES)), size = 0.75, color = "navy") + 
  geom_vline(xintercept = todays_date, linetype = 3) +
