...
reddCount <- 354
countDate <- "August 31, 2023"
updatedReddInfoDate <- "September 18, 2023"
...
scenarios <- c("Alt.1b", "Alt.2d", "Alt.2e", "Alt.3i", "Alt.3j", "Alt.3k")
...
reddsTable <- reddsTable %>% rename('Date' = 'Estimated.Date.of.Emergence', 'Flow' = 'ACTUAL.or.ESTIMATED..DEWATER.FLOW') %>% mutate(cols = factor(cols, levels = c('Emerged', 'Dewatered', 'Remaining')))
...
ggplot() + geom_line(flows, mapping = aes(x = Date, y = Flow, color = Alts), size = 0.75) +
  ...
  scale_fill_manual(labels = c('Emerged', 'Dewatered', 'Remaining'), values = c('#33CCFF', '#CC6600', 'darkgrey')) +
  annotate(geom = 'rect', xmin = as.Date('2023-09-01'), xmax = max(todays_date), 
           ymin = 4000, ymax = 10000, fill = 'darkgrey', color = 'black', alpha = 0.2, linetype = 'dotted') +
