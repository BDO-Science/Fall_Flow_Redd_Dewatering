...
#KWK daily flows url.  This url will have to be updated every year.
kwk_webpage <- read_html('https://www.cbr.washington.edu/sacramento/tmp/riverdaily_1694026523_22.html')
...
kes_webpage <- read_html('https://www.cbr.washington.edu/sacramento/tmp/riverdaily_1693953038_980.html')
...
redd_model <- kesFlowReal %>% 
  gather(key = Scenarios, value = Spawn_Flows, A.1bcfs, A.3ccfs, A.3dcfs, A.3fcfs, A.3gcfs) %>%
...
redd_model <- redd_model %>% left_join(Spawn_info, by = 'Date') %>% group_by(Scenarios) %>%
  ...
  mutate('100cfs' = Dewater + 100, '200cfs' = Dewater + 250) %>% 
  gather(key = 'Buffer', value = 'Dewater', c('Dewater', '100cfs', '200cfs'))
...
summary <- redd_model %>% mutate(Redds_lost = pred * Per_spawn) %>% group_by(Scenarios, Buffer) %>% 
  summarize(Redds_lost = round((sum(Redds_lost)*100), 1)) %>% spread(key = Buffer, value = Redds_lost) %>%
  select(Dewater, '100cfs', '200cfs') %>%
  rename('Fall-run dewatered (%)' = 'Dewater', 'Fall-run dewatered (100 cfs buffer; %)' =  '100cfs',
  'Fall-run dewatered (250 cfs buffer; %)' = '200cfs')
