...
updatedReddInfoDate <- "September 10, 2023"
...
scenarios <- c("Alt.2d", "Alt.3g", "Alt.3h", "Alt.3i", "Alt.3j")
...
cutoff_date <- as.Date(max(kwk_flows$Date))
...
Metric <- c("Avg Sept Flow (cfs)", "Avg Oct Flow (cfs)", "Sept-Feb Total Volume (TAF)", "Aug-Oct Total Volume (TAF)", "Winter-run Redds dewatered", "Winter-run Percent Lost (based on current count)", paste0("Winter-run Percent Lost (based on this year's anticipated expansion of ", yr_exp, ")"), "Winter-run Percent Lost (based on average expansion of 1.98)", "Winter-run Percent Lost (based on conservative expansion of 1.25)", "Winter-run Redds Dewatered (w/ 250 cfs buffer)", "Winter-run Percent Lost (w/ 250 cfs buffer)")
...
for(a in 1:(length(kesFlow)-1)){
 ...
 dflist[[a]] <- as.character(c(avg_sept_flow, avg_oct_flow, total_volume_sf, total_volume_ao, 
                               wr_dewatered, wr_pct_lost, wr_pct_lost_300, wr_pct_lost_200, wr_pct_lost_25,
...
todays_date <- as.Date(max(kwk_flows$Date, kes_flows$Date))
